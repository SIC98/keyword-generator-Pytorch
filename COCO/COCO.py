...
class COCO(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        annotation_file = os.path.join(dl_manager.download_and_extract(
            _KARPATHY_FILES_URL), "dataset_coco.json")
    ...
    def _generate_examples(self, annotation_file, split_key):
        if self.config.name == "2014_captions":
            return self._generate_examples_2014_captions(annotation_file, split_key)
        elif self.config.name == "2014":
            return self._generate_examples_2014(annotation_file, split_key)

    def _generate_examples_2014_captions(self, annotation_file, split_key):
    ...
    def _generate_examples_2014(self, annotation_file, split_key):
        counter = 0
        with open(annotation_file, "r", encoding="utf-8") as fi:
            annotations = json.load(fi)
